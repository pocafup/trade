...
function MarketPulse({ ticks }: { ticks: MarketTick[] }) {
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        <TrendingUp size={13} className="text-[#4F8EF7]" />
        <p className="text-[10px] font-medium text-[#6B7E9C] uppercase tracking-wider">市场脉搏 · 今日</p>
      </div>
      {ticks.length === 0 ? (
        <div className="rounded-2xl bg-[#0F1520] border border-[#1E2D42] p-4 text-xs text-[#3A4E6A]">
          行情数据暂不可用，稍后刷新
        </div>
      ) : (
          ...
          {ticks.map((t) => {
            ...
            return (
              <div key={t.symbol} className="bg-[#0F1520] border border-[#1E2D42] rounded-2xl p-3">
                <p className="text-[10px] text-[#6B7E9C] truncate mb-1">{t.label}</p>
                <p className="font-mono font-bold text-base text-[#E8EDFB] tracking-tight leading-none">
                  {fmtPrice(t)}
                </p>
...
function FearGreedCard({ fg }: { fg: FearGreed | null }) {
    ...
    return (
      <section>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[10px] font-medium text-[#6B7E9C] uppercase tracking-wider">市场情绪</p>
        </div>
        <div className="rounded-2xl bg-[#0F1520] border border-[#1E2D42] p-4 text-xs text-[#3A4E6A]">
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        <Gauge size={13} className="text-[#4F8EF7]" />
        <p className="text-[10px] font-medium text-[#6B7E9C] uppercase tracking-wider">市场情绪 · CNN 恐惧贪婪指数</p>
      </div>
      <div className="rounded-2xl bg-[#0F1520] border border-[#1E2D42] p-5">
        <div className="flex items-end gap-4 mb-4">
          <div>
            <p className="font-mono font-bold text-4xl leading-none tracking-tight" style={{ color }}>
              {fg.score}
            </p>
            <p className="text-sm font-semibold mt-1.5" style={{ color }}>{fg.label}</p>
          </div>
          <p className="text-[11px] text-[#6B7E9C] leading-relaxed pb-1 flex-1">
        ...
        <div className="flex justify-between text-[9px] text-[#3A4E6A] mt-2 mb-4">
        ...
        <div className="grid grid-cols-4 gap-2 pt-3 border-t border-[#1E2D42]">
          {trends.map((tr) => (
            <div key={tr.label}>
              <p className="text-[10px] text-[#6B7E9C] mb-1">{tr.label}</p>
              <div className="flex items-baseline gap-1">
                <span className="font-mono text-sm font-semibold" style={{ color: fgColor(tr.v) }}>{tr.v}</span>
                <span className="text-[9px]" style={{ color: fgColor(tr.v) }}>
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="text-[10px] font-medium text-[#6B7E9C] uppercase tracking-wider">经济日历 · 即将公布</p>
      </div>
        ...
        {events.map((e, i) => (
          <div key={e.label} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? 'border-t border-[#1E2D42]' : ''}`}>
            <div className="flex flex-col items-center justify-center w-14 shrink-0">
              <span className={`font-mono font-bold text-lg leading-none ${e.daysAway <= 3 ? 'text-[#4F8EF7]' : 'text-[#E8EDFB]'}`}>
                {e.daysAway}
              </span>
              <span className="text-[9px] text-[#3A4E6A] mt-0.5">天后</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-[#E8EDFB]">{e.label}</p>
              <p className="text-[11px] text-[#6B7E9C] mt-0.5">{e.note}</p>
            </div>
            <span className="font-mono text-xs text-[#8B9CC0] shrink-0">{e.date}</span>
          </div>
        ))}
      </div>
    </section>
...
function IndicatorsCard({ items }: { items: EconIndicator[] }) {
  if (!items.length) {
    return (
      <section>
        <div className="flex items-center gap-2 mb-3">
          <Landmark size={13} className="text-[#4F8EF7]" />
          <p className="text-[10px] font-medium text-[#6B7E9C] uppercase tracking-wider">经济数据</p>
        </div>
        <div className="rounded-2xl bg-[#0F1520] border border-[#1E2D42] p-4 text-xs text-[#3A4E6A]">
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        <Landmark size={13} className="text-[#4F8EF7]" />
        <p className="text-[10px] font-medium text-[#6B7E9C] uppercase tracking-wider">经济数据 · 最新读数</p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
        {items.map((it) => {
          ...
          return (
            <div key={it.key} className="bg-[#0F1520] border border-[#1E2D42] rounded-2xl p-3" title={it.hint}>
              <p className="text-[10px] text-[#6B7E9C] truncate mb-1">{it.label}</p>
              <div className="flex items-baseline gap-1">
                <p className="font-mono font-bold text-xl text-[#E8EDFB] tracking-tight leading-none">
                  {it.value.toFixed(it.unit === '%' ? 1 : 2)}
                </p>
                <span className="text-[11px] text-[#6B7E9C]">{it.unit}</span>
              </div>
              <div className="flex items-center justify-between mt-1.5">
                {delta != null ? (
                  <span className={`font-mono text-[11px] ${arrowColor}`}>
                    {delta > 0 ? '▲' : delta < 0 ? '▼' : '–'} {Math.abs(delta).toFixed(2)}
                  </span>
                ) : <span />}
                <span className="text-[9px] text-[#3A4E6A]">{it.date}</span>
              </div>
            </div>
          );
        })}
...
function FomcCard({ fomc }: { fomc: FomcSummary[] }) {
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        <Landmark size={13} className="text-[#4F8EF7]" />
        <p className="text-[10px] font-medium text-[#6B7E9C] uppercase tracking-wider">美联储 FOMC 声明 · AI 摘要</p>
      </div>
          ...
          {fomc.map((s, i) => (
            <div key={s.url || i} className="rounded-2xl bg-[#0F1520] border border-[#1E2D42] overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-[#1E2D42]">
                ...
                <span className="font-mono text-xs text-[#8B9CC0]">{s.date || '—'}</span>
                <a href={s.url} target="_blank" rel="noopener noreferrer"
                  className="ml-auto flex items-center gap-1 text-[10px] text-[#6B7E9C] hover:text-[#4F8EF7] transition-colors">
              ...
              <div className="px-4 py-3">
                {s.summary ? (
                  <p className="text-sm text-[#C7D2E6] leading-relaxed">{s.summary}</p>
                ) : (
                  <p className="text-xs text-[#3A4E6A]">AI 摘要暂不可用，点击右上角查看官方原文。</p>
                )}

                {i === 0 && s.changes.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-[#1E2D42]">
                    <p className="text-[10px] text-[#6B7E9C] uppercase tracking-wider mb-2">较上次的变化</p>
                    <ul className="space-y-1.5">
                      {s.changes.map((c, j) => (
                        <li key={j} className="flex gap-2 text-xs text-[#8B9CC0] leading-relaxed">
...
export default function MacroPage() {
  ...
  return (
    <div className="min-h-screen bg-[#080B14]">
      ...
      <main className="max-w-5xl mx-auto px-4 pb-24 pt-6 space-y-6">
        {loading && !data ? (
          <Skeleton />
        ) : err && !data ? (
          <div className="rounded-2xl bg-rose-950/30 border border-rose-500/30 p-6 flex items-center gap-2">
            <AlertTriangle size={15} className="text-rose-400 shrink-0" />
            <p className="text-sm text-rose-200/80">获取宏观数据失败，请点击右上角刷新重试。</p>
          </div>
        ) : data ? (
          <>
            <MarketPulse ticks={data.marketPulse} />
            <FearGreedCard fg={data.fearGreed} />
            <CalendarCard events={data.calendar} />
            <IndicatorsCard items={data.indicators} />
            <FomcCard fomc={data.fomc} />
            <p className="text-[10px] text-[#3A4E6A] text-center pt-1 leading-relaxed">
